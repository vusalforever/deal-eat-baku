...
export function EmojiCategoryBar({
  ...
  return (
    <div
      className="flex gap-3 sm:gap-4 overflow-x-auto py-2 px-1 -mx-1"
      style={{ scrollbarWidth: "none" }}
    >
      <button
        onClick={() => onSelect(undefined)}
        className="shrink-0 flex flex-col items-center gap-1 sm:gap-1.5 group"
      >
        <span
          className={`grid place-items-center size-12 sm:size-16 rounded-full text-2xl sm:text-3xl transition-all ${
            allActive
        ...
        <span
          className={`text-[10px] sm:text-xs font-semibold transition ${
      ...
      {categories.map((c) => {
        ...
        return (
          ...
          >
            <span
              className={`grid place-items-center size-12 sm:size-16 rounded-full text-2xl sm:text-3xl transition-all ${c.bg} ${
                isActive
